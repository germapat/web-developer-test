<template>
  <v-layout row justify-center>
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        <v-card-title>
          <span class="headline">{{$t('task.create-task')}}</span>
        </v-card-title>
              ...
              <v-layout wrap>
                ...
                <v-flex xs12 sm6 md12>
                  ...
                  >
                    ...
                    <v-date-picker color="primary" :rules="[rules.required]" v-model.trim="expirationDate" @input="menu = false"></v-date-picker>
                  </v-menu>
                </v-flex>
              </v-layout>
            </v-container>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat @click.native="submit">{{$t('save-button-label')}}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
</template>
<script src="./taskCreate.js"></script>